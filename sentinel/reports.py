...
def _spend_where(prefix: str = "") -> str:
    ...
    return (
        f"{prefix}amount_cents < 0 AND {prefix}category NOT IN ({non_spend}) "
        ...
        out.append(
            {
                ...
                "spend_cents": r["spend"],
                "txns": r["n"],
                "pct": 100.0 * r["spend"] / total,
                "cum_pct": 100.0 * running / total,
            }
        )
    return out
...
def merchant_pareto(conn, start: str, end: str, top: int = 25) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        "GROUP BY v.merchant_id ORDER BY spend DESC LIMIT ?",
        (start, end, top),
    ).fetchall()
    total_row = conn.execute(
        f"SELECT COALESCE(SUM(-amount_cents), 0) FROM v_transactions_categorized WHERE {_spend_where()}", (start, end)
    ).fetchone()
...
def size_bands(conn, start: str, end: str) -> list[dict[str, Any]]:
    ...
    for lo, hi, label in SIZE_BANDS:
        ...
        row = conn.execute(
            ...
            f"FROM v_transactions_categorized WHERE {_spend_where()} {clause}",
            params,
        ).fetchone()
...
def weekday_profile(conn, start: str, end: str) -> dict[str, Any]:
    rows = conn.execute(
        ...
        f"FROM v_transactions_categorized WHERE {_spend_where()} GROUP BY dow",
        (start, end),
    ).fetchall()
        ...
        out.append(
            {
                "month": r["month"],
                "full_month": full,
                "inflows_cents": r["inflows"],
                "fixed_cents": r["fixed"],
                "variable_cents": r["variable"],
                "uncategorized_cents": r["uncategorized"],
                "transfers_out_cents": r["transfers_out"],
                "net_cents": r["net"],
...
def detect_recurring(conn, cfg: dict[str, Any], as_of: date) -> list[dict[str, Any]]:
    ...
    for r in rows:
        g = groups.setdefault(
            r["merchant_id"], {"name": r["name"], "first_seen": r["first_seen"], "dates": [], "amounts": []}
        )
...
def chart_category_pareto(outdir: Path, pareto) -> str | None:
    ...
    ax2.plot(labels, [p["cum_pct"] for p in pareto], color=BLUE, linewidth=2, marker="o", markersize=4)
    ax2.set_ylabel("cumulative %", color=INK_2, fontsize=9)
    ax2.set_ylim(0, 105)
    ax2.axhline(80, color=NEUTRAL, linewidth=1, linestyle="--")
    ax2.annotate(
        "80%", xy=(len(labels) - 1, 80), color=INK_2, fontsize=8, xytext=(0, 4), textcoords="offset points", ha="right"
    )
    for ax in (ax1, ax2):
...
def chart_merchant_pareto(outdir: Path, merchants) -> str | None:
    ...
    ax.barh([m["merchant"] for m in rows], [m["spend_cents"] / 100 for m in rows], color=BLUE, height=0.62)
    ax.xaxis.grid(True, color=GRID, linewidth=0.8)
    ax.yaxis.grid(False)
    for i, m in enumerate(rows):
        ax.annotate(
            f" {fmt_eur(m['spend_cents'])}", xy=(m["spend_cents"] / 100, i), va="center", color=INK_2, fontsize=7
        )
    ax.set_title(f"Top {len(merchants)} merchants — trailing 90d spend", color=INK, fontsize=11, loc="left")
...
def chart_size_bands(outdir: Path, bands) -> str | None:
    ...
    ax1.set_title("Transaction size bands — count vs total (small-taps check)", color=INK, fontsize=11, loc="left")
    ax1.set_ylabel("count", color=INK_2, fontsize=9)
    for i, b in enumerate(bands):
        ax1.annotate(str(b["count"]), xy=(i, b["count"]), ha="center", va="bottom", color=INK_2, fontsize=8)
    ax2.bar(labels, [b["total_cents"] / 100 for b in bands], color=BLUE, width=0.62)
    ax2.set_ylabel("total €", color=INK_2, fontsize=9)
    for i, b in enumerate(bands):
        ax2.annotate(
            fmt_eur(b["total_cents"]), xy=(i, b["total_cents"] / 100), ha="center", va="bottom", color=INK_2, fontsize=8
        )
...
def chart_weekday(outdir: Path, profile) -> str | None:
    ...
    ax.bar([d["day"] for d in days], [d["spend_cents"] / 100 for d in days], color=colors, width=0.62)
    for i, d in enumerate(days):  # value label on every bar
        ax.annotate(
            fmt_eur(d["spend_cents"]), xy=(i, d["spend_cents"] / 100), ha="center", va="bottom", color=INK_2, fontsize=8
        )
...
def chart_burn_rate(outdir: Path, daily, budget_cents: int) -> str | None:
    ...
    for idx, month in enumerate(months):
        ...
        ax.annotate(
            f" {month}", xy=(xs[-1], ys[-1]), color=SERIES[idx], fontsize=8, va="center"
        )  # end labels, so the line hue isn't load-bearing
    ...
    ax.plot(
        ...
        linewidth=1.4,
        linestyle="--",
        label="straight-line budget",
    )
    ax.set_title("Cumulative spend per month vs straight-line budget", color=INK, fontsize=11, loc="left")
...
def render_subscriptions_md(recurring, as_of: date, min_n: int = 3) -> str:
    ...
    lines.append(f"**{len(recurring)} recurring merchant(s) · {fmt_eur(total_annual)}/year if all continue.**\n")
    lines.append(
        _md_table(
            ...
            [
                [
                    s["merchant"],
                    fmt_eur(s["amount_cents"]),
                    s["period"],
                    s["next_expected"],
                    fmt_eur(s["annualized_cents"]),
                    s["first_seen"] or "—",
                    str(s["occurrences"]),
                ]
                for s in recurring
...
def render_report_md(data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        _md_table(
            ["Category", "Spend", "% of total", "Cumulative %", "Txns"],
            [
                [c["category"], fmt_eur(c["spend_cents"]), f"{c['pct']:.1f}%", f"{c['cum_pct']:.1f}%", str(c["txns"])]
        ...
        "",
        _md_table(
            ["Merchant", "Category", "Spend", "% of total", "Txns"],
            [
                [m["merchant"], m["category"], fmt_eur(m["spend_cents"]), f"{m['pct']:.1f}%", str(m["txns"])]
        ...
        _md_table(
            ["Band", "Count", "Total"],
            [[b["band"], str(b["count"]), fmt_eur(b["total_cents"])] for b in data["size_bands"]],
        ),
        "",
        ...
        _md_table(
            ["Day", "Spend", "Txns"],
            [[d["day"], fmt_eur(d["spend_cents"]), str(d["txns"])] for d in data["weekday"]["days"]],
        ),
        "",
        "## 5. Burn rate",
        "",
        ...
        "",
        f"Straight-line reference: {fmt_eur(data['budget_cents'])}/month ({data['budget_source']}).",
        "",
        "## 6. Recurring spend",
        "",
        f"{len(data['recurring'])} recurring merchant(s) detected — see [subscriptions.md](subscriptions.md).",
        "",
        ...
        _md_table(
            [
                ...
                "Net saved",
                "Residual",
                f"< {fmt_eur(RESIDUAL_GATE_CENTS)}?",
            ],
            [
                [
                    m["month"] + ("" if m["full_month"] else " (partial)"),
                    fmt_eur(m["inflows_cents"]),
                    fmt_eur(m["fixed_cents"]),
                    fmt_eur(m["variable_cents"]),
                    fmt_eur(m["uncategorized_cents"]),
                    fmt_eur(m["transfers_out_cents"]),
                    fmt_eur(m["net_cents"]),
        ...
        (
            _md_table(
                ["Bucket", data["mom"]["prev"], data["mom"]["curr"], "Δ€", "Δ%"],
                [
                    [
                        r["bucket"],
                        fmt_eur(r["prev_cents"]),
                        fmt_eur(r["curr_cents"]),
                        fmt_eur(r["delta_cents"]),
                        f"{r['delta_pct']:+d}%",
                    ]
                    for r in data["mom"]["rows"]
                ],
            )
            if data["mom"]["rows"]
    ...
    if full_months:
        ...
        verdict = "✅ GATE PASS" if all(m["gate_pass"] for m in full_months) else "❌ GATE FAIL"
        lines.append(
            f"**{verdict}** — worst full-month |residual| {fmt_eur(worst)} "
...
def month_over_month(conn, as_of: date, inflow_exclude_cents: int = 0) -> dict[str, Any]:
    ...
    for b in BUCKETS:
        ...
        rows.append({"bucket": b, "curr_cents": c, "prev_cents": p, "delta_cents": c - p, "delta_pct": pct})
    return {"curr": curr_key, "prev": prev_key, "rows": rows}


def run_reports(conn, cfg: dict[str, Any], as_of: date | None = None, dry_run: bool = False) -> dict[str, Any]:
    ...
    full_totals = [m["fixed_cents"] + m["variable_cents"] + m["uncategorized_cents"] for m in months if m["full_month"]]
    ...
    if full:
        ...
        log.info(
            "gap residual: worst full month %s (gate %s at %s)",
            fmt_eur(worst),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--as-of", default=None, metavar="ISO_DATE", help="window end date (default: today, Europe/Dublin)"
    )
