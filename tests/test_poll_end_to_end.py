...
def env(tmp_path, monkeypatch):
    ...
    def fake_run_ingest(conn_, client, uids, **kw):
        conn_.execute("INSERT OR IGNORE INTO merchants (name_normalized, category, categorized_by) "
                      "VALUES ('DELIVEROO', 'FoodDelivery', 'dict')")
        mid = conn_.execute("SELECT id FROM merchants WHERE name_normalized='DELIVEROO'").fetchone()[0]
        db.insert_transactions(conn_, [{"account_id": "a", "booking_date": AS_OF.isoformat(),
                                        "amount_cents": -20000, "merchant_raw": "DELIVEROO",
                                        "merchant_id": mid, "source": "api"}])
        return 1, 1
