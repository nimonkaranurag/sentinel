...
def env(tmp_path, monkeypatch):
    ...
    db.init_db(conn)
    cur = conn.execute(
        "INSERT INTO merchants (name_normalized, category, categorized_by) VALUES ('DELIVEROO', 'FoodDelivery', 'dict')"
    )
...
def test_alert_carries_keyboard_and_records_event(env):
    ...
    mid = alerts.send_policy_alert(conn, {"txn_id": tid, "policy": "food-delivery", "text": "🔴 Deliveroo over cap"})
    kb = fake.sent[0]["reply_markup"]["inline_keyboard"]
    assert kb[0][0]["text"] == "✓ fine" and kb[0][1]["text"] == "Reclassify…"
    assert tuple(conn.execute("SELECT kind, txn_id, status FROM events").fetchone()) == ("policy_alert", tid, "sent")
    assert mid == 101
...
def test_reclassify_round_trip_edits_and_teaches(env):
    ...
    assert (
        conn.execute("SELECT category, categorized_by FROM merchants WHERE name_normalized = 'DELIVEROO'").fetchone()[0]
        == "Groceries"
    )
    assert conn.execute("SELECT status FROM events WHERE message_id = ?", (mid,)).fetchone()[0] == "reclassified"
