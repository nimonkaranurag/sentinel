...
_CENTS = [0, 1, -1, 5, -5, 99, 100, -100, 4567, -4567, 100_099, 1_368_00, 100_000_099, -100_000_099] + list(
    range(-250, 250, 7)
)
...
_GENERATED = [f"{p}{b}{t}{c}" for p in _PREFIXES for b in _BRANDS for t in _TRAILERS for c in _CITIES]
...
def test_hash_id_occurrence_is_permutation_stable():
    ...
    a = {
        ...
        "amount_cents": -350,
        "merchant_raw": "COFFEE ANGEL",
        "source": "csv",
    }
    b = dict(a)  # byte-identical → occurrence disambiguates
    c = {"account_id": "x", "booking_date": "2026-01-03", "amount_cents": -500, "merchant_raw": "LIDL", "source": "csv"}
